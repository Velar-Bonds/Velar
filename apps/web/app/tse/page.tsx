...
const nav = [
  ...
  { icon: 'assignment_turned_in', label: 'Revisión' },
  { icon: 'send', label: 'Emisión' },
  { icon: 'list_alt', label: 'Registros' },
  { icon: 'account_tree', label: 'Trazabilidad' },
  { icon: 'search', label: 'Auditoría' },
  { icon: 'public', label: 'Publicación' },
];
...
export default function TSEPage() {
  ...
  const statusChip = (s: string) => ({ activo: 'bg-emerald-50 text-emerald-700 border-emerald-200', en_escrow: 'bg-amber-50 text-amber-700 border-amber-200', congelado: 'bg-red-50 text-red-600 border-red-200', emitido: 'bg-blue-50 text-primary border-blue-200' } as any)[s] ?? 'bg-gray-100 text-gray-600 border-gray-200';

  const metrics = [
    ['Pendientes de revisión', pendientes, 'description', 'text-primary', 'bg-primary-container/10'],
    ['Movimientos liberados', aprobados, 'check_circle', 'text-green-600', 'bg-green-50'],
    ['Bonos emitidos', bonds.length, 'send', 'text-blue-500', 'bg-blue-50'],
    ['Transferencias totales', transfers.length, 'trending_up', 'text-teal-500', 'bg-teal-50'],
  ];
      ...
      <aside className="glass-sidebar sticky top-0 z-40 hidden h-full w-64 shrink-0 flex-col justify-between md:flex">
        <div className="px-6 py-8">
          <div className="mb-10 flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded bg-primary"><span className="material-symbols-outlined text-white" style={{ fontVariationSettings: "'FILL' 1" }}>change_history</span></div>
            <span className="text-2xl font-semibold tracking-tight text-primary">VELAR</span>
          </div>
          <nav className="space-y-2">
            {nav.map((n) => (
              <a key={n.label} href="#" className={`flex items-center gap-3 rounded-lg px-4 py-3 font-medium transition-colors ${n.active ? 'bg-primary-container/10 text-primary' : 'text-on-surface-variant hover:bg-surface-variant/50'}`}>
                <span className="material-symbols-outlined" style={n.active ? { fontVariationSettings: "'FILL' 1" } : {}}>{n.icon}</span>{n.label}
              </a>
            ))}
          </nav>
        </div>
        <div className="px-6 py-6">
          <div className="glass-card mb-6 rounded-xl p-4">
            <div className="mb-2 flex items-center gap-2"><span className="material-symbols-outlined text-green-600" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span><span className="text-sm font-semibold">Sistema verificado</span></div>
      ...
      <main className="flex h-full flex-1 flex-col overflow-y-auto">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-surface-variant/50 bg-[#FAFCFF]/80 px-8 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4">
            <div className="flex cursor-pointer items-center gap-3 border-l border-surface-variant pl-4">
              ...
              <button onClick={async () => { await supabase.auth.signOut(); router.push('/login'); }} className="material-symbols-outlined text-outline hover:text-primary">logout</button>
            </div>
          </div>
        </header>
        ...
        <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-6 p-8">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {metrics.map(([label, val, icon, color, bg]) => (
              <div key={label as string} className="glass-card flex items-center gap-4 rounded-xl p-4 transition-transform duration-300 hover:-translate-y-1">
                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ${bg}`}><span className={`material-symbols-outlined ${color}`}>{icon}</span></div>
                <div><p className="text-sm font-medium text-on-surface-variant">{label}</p><span className="text-lg font-bold">{val}</span></div>
              </div>
            ))}
          </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="glass-card rounded-xl bg-white/50 p-4">
                ...
                <div className="flex flex-col gap-3">
                  {[['post_add', 'Emitir registro', 'Crear y emitir un nuevo bono', 'primary'], ['search', 'Ver auditoría', 'Consultar eventos auditables', 'teal-500'], ['cloud_upload', 'Publicar registro', 'Publicar en el portal público', 'purple-500']].map(([ic, t, d]) => (
                    <button key={t} className="group flex items-center justify-between rounded-xl border border-[#D5E3FF] bg-white p-3 text-left transition-all hover:border-primary hover:shadow-md">
                      <div className="flex items-center gap-4"><div className="flex h-10 w-10 items-center justify-center rounded bg-primary/10 text-primary"><span className="material-symbols-outlined">{ic}</span></div><div><p className="text-sm font-medium text-on-surface">{t}</p><p className="text-xs text-on-surface-variant">{d}</p></div></div>
                ...
                <div className="relative space-y-4 before:absolute before:bottom-2 before:left-4 before:top-2 before:w-0.5 before:bg-surface-variant/50">
                  {transfers.slice(0, 6).map((t) => (
                    <div key={t.id} className="relative flex items-start gap-4">
                      <div className="relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-blue-200 bg-blue-50 text-blue-500 shadow-sm"><span className="material-symbols-outlined text-[18px]">swap_horiz</span></div>
