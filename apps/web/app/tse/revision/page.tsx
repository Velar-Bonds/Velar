...
import { SEED_REQUESTS, SEED_PARTIES } from '../../../lib/tse-seed';
...
type Request = {
  ...
  notes?: string; rejection_reason?: string; party_id?: string;
...
export default function RevisionPage() {
  ...
  const load = (tok: string) =>
    apiFetch(tok, 'GET', '/bonds/requests').then(setRequests).catch(() => setRequests(SEED_REQUESTS));
  ...
  async function approve(id: string) {
    setBusy(id); setMsg('');
    try { await apiFetch(token, 'PATCH', `/bonds/requests/${id}/approve`); setMsg('✅ Bono aprobado y emitido al partido.'); load(token); }
    catch (e: any) { setMsg('⚠️ ' + e.message); } finally { setBusy(null); }
  }

  async function reject(id: string) {
    if (!rejectReason.trim()) { setMsg('Ingresá el motivo del rechazo.'); return; }
    setBusy(id); setMsg('');
    try { await apiFetch(token, 'PATCH', `/bonds/requests/${id}/reject`, { reason: rejectReason }); setMsg('Solicitud rechazada.'); setRejectTarget(null); setRejectReason(''); load(token); }
    catch (e: any) { setMsg('⚠️ ' + e.message); } finally { setBusy(null); }
  }
  ...
  const filtered = requests.filter((r) => {
    ...
    if (search) {
      ...
      if (!r.certificate_number?.toLowerCase().includes(q) && !r.parties?.name?.toLowerCase().includes(q) && !r.series?.toLowerCase().includes(q)) return false;
    }
    return true;
  });

  const pendCount = filtered.filter((r) => r.status === 'pendiente').length;
  ...
  return (
    <TSEShell me={me}>
      <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-surface-variant/40 bg-[#FAFCFF]/85 px-8 backdrop-blur-md">
        <div>
          <h1 className="text-2xl font-bold" style={{ fontFamily: 'Geist' }}>Revisión de solicitudes</h1>
          <p className="text-sm text-on-surface-variant">{pendCount} pendientes de revisión</p>
        </div>
      </header>
        ...
        <div className="glass-card overflow-hidden rounded-2xl">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-surface-variant/30 bg-surface-container-low/50 text-[11px] font-semibold uppercase tracking-wide text-on-surface-variant">
              <tr>
                <th className="px-5 py-3">ID de solicitud</th>
            ...
            <tbody className="divide-y divide-surface-variant/20">
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="py-10 text-center text-on-surface-variant">Sin solicitudes con estos filtros.</td></tr>
              )}
              {filtered.map((r) => {
                ...
                return (
                  <Fragment key={r.id}>
                    ...
                    {/* Detalle expandido */}
                    {isExpanded && (
                      <tr key={`${r.id}-detail`} className="bg-surface-container-low/30">
                        <td colSpan={7} className="px-5 py-4">
                          <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                            {[
                              ...
                              ['Serie', r.series ?? '—'],
                              ['Moneda', r.currency],
                              ['Tasa de interés', r.interest_rate != null ? `${r.interest_rate}%` : '—'],
