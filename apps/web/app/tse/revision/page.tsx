...
export default function RevisionPage() {
  ...
  const load = (tok: string) =>
    apiFetch(tok, 'GET', '/bonds/requests').then(setRequests).catch(() => setRequests([]));
  ...
  async function reject(id: string) {
    if (!rejectReason.trim()) { notify.err('Ingresá el motivo del rechazo.'); return; }
    setBusy(id); 
    try { await apiFetch(token, 'PATCH', `/bonds/requests/${id}/reject`, { reason: rejectReason }); notify.ok('Solicitud rechazada.'); setRejectTarget(null); setRejectReason(''); load(token); }
    catch (e: any) { notify.err(e.message); } finally { setBusy(null); }
  }

  const parties = Array.from(new Set(requests.map((r: any) => r.parties?.name).filter(Boolean))) as string[];
